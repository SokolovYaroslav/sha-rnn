...
parser.add_argument("--warmup", type=int, default=4000, help="warmup for learning rate")
parser.add_argument("--cooldown", type=int, default=None, help="cooldown for learning rate")
parser.add_argument("--accumulate", type=int, default=1, help="number of batches to accumulate before gradient update")
...
parser.add_argument(
    "--wdrop", type=float, default=0.0, help="amount of weight dropout to apply to the RNN hidden to hidden matrix"
)
...
def evaluate(data_source, batch_size=10):
    ...
    model.eval()
    if args.model == "QRNN" and getattr(model, "reset", None):
        model.reset()
    total_loss = 0
    ntokens = len(corpus.dictionary)
    hidden = None
    ...
    while i < train_data.size(0) - 1 - 1:
        ...
        if args.wdrop:
            ...
            for b in model.blocks:
                ...
                m = (
                    (1 - args.wdrop) * torch.ones(b.rnn.weight_hh_l0.shape, device=b.rnn.weight_hh_l0.device)
                ).bernoulli()
    ...
    for epoch in range(1, args.epochs + 1):
        ...
        if "t0" in optimizer.param_groups[0]:
            ...
            if (
                ...
                optimizer = torch.optim.ASGD(model.parameters(), lr=args.lr, t0=0, lambd=0.0, weight_decay=args.wdecay)
